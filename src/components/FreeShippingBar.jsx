...
export default function FreeShippingBar({ currentAmount = 0, threshold = 499, className = '' }) {
  ...
  return (
    <div className={`w-full bg-white dark:bg-[#0F172A] p-3.5 sm:p-4 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-2xs transition-colors ${className}`}>
      <div className="flex items-center justify-between text-xs sm:text-sm font-bold mb-2">
        <span className="flex items-center gap-1.5 text-gray-800 dark:text-gray-100">
          <Truck className={`w-4 h-4 ${isUnlocked ? 'text-[#70BF4F]' : 'text-amber-500 dark:text-amber-400'}`} />
          {isUnlocked ? (
            <span className="text-[#70BF4F] dark:text-[#86EFAC] flex items-center gap-1 font-extrabold">
              <CheckCircle2 className="w-4 h-4" /> You've unlocked FREE Delivery across Maharashtra!
            </span>
          ) : (
            <span>
              Add <strong className="text-[#70BF4F] dark:text-[#86EFAC]">₹{remaining}</strong> more for{' '}
      ...
      <div className="w-full h-2.5 bg-gray-100 dark:bg-[#1E293B] rounded-full overflow-hidden relative">
        <div
      ...
      {isUnlocked && (
        <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1.5 flex items-center gap-1">
          <Sparkles className="w-3 h-3 text-[#70BF4F] dark:text-[#86EFAC]" /> Express dispatch from Pune kitchen within 24 hours.
        </p>
      )}
    </div>
  );
